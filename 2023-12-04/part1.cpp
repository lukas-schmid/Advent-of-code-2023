...
void resultPart1()
{
    ...
    while (getline(inputFile, line))
    {
        ...
        size_t indexOfPipe = line.find('|');

        std::vector<int> numbers;
        std::vector<int> winningNumbers;

        extractNumbersByRange(line, indexOfColon, indexOfPipe, numbers);
        extractNumbersByRange(line, indexOfPipe, line.size(), winningNumbers);
        ...
        for (int num : numbers)
        {
            ...
            if (it != winningNumbers.end())
            {
                ...
                {
                    int lastInt = points.back();
                    points.push_back(lastInt * 2);
                }
            }
        }

        int sumPoints = std::accumulate(points.begin(), points.end(), 0);
        sum += sumPoints;
    }
